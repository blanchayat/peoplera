...
function computeBurnoutScoreFromWeeklyMetrics(m){
  const weeklyHours = Number(m?.weekly_hours ?? m?.weeklyHours ?? 0);
  const weekendHours = Number(m?.weekend_hours ?? m?.weekendHours ?? 0);
  const afterHoursMessages = Number(m?.after_hours_messages ?? m?.afterHoursMessages ?? 0);
  const sickDays = Number(m?.sick_days ?? m?.sickDays ?? 0);

  const hoursComponent = clamp((weeklyHours - 40) * 2.0, 0, 40);
  const weekendComponent = clamp(weekendHours * 3.0, 0, 20);
  const afterHoursComponent = clamp(afterHoursMessages * 1.5, 0, 20);
  const sickComponent = clamp(sickDays * 6.0, 0, 20);

  return Math.round(clamp(hoursComponent + weekendComponent + afterHoursComponent + sickComponent, 0, 100));
}
...
function computeTopDriverFromMetrics(metrics){
  const list = Array.isArray(metrics) ? metrics : [];
  const counts = { 'Workload (weekly hours)': 0, 'Weekend work': 0, 'After-hours messages': 0, 'Sick days': 0 };
  for (const m of list) {
    const weeklyHours = Number(m?.weekly_hours ?? 0);
    const weekendHours = Number(m?.weekend_hours ?? 0);
    const afterHoursMessages = Number(m?.after_hours_messages ?? 0);
    const sickDays = Number(m?.sick_days ?? 0);
    const contributions = [
      { k: 'Workload (weekly hours)', v: clamp((weeklyHours - 40) * 2.0, 0, 40) },
      { k: 'Weekend work', v: clamp(weekendHours * 3.0, 0, 20) },
      { k: 'After-hours messages', v: clamp(afterHoursMessages * 1.5, 0, 20) },
      { k: 'Sick days', v: clamp(sickDays * 6.0, 0, 20) }
    ];
    contributions.sort((a,b)=>b.v-a.v);
    const top = contributions[0];
    if (top?.k) counts[top.k] = (counts[top.k] || 0) + 1;
  }
...
async function loadBurnoutMain(){
  ...
  out.innerHTML = `
    <div style="display:grid;gap:12px">
      <div id="burnoutTeamSection"></div>
      <div id="burnoutWeekSection"></div>
      <div id="burnoutScoreSection"></div>
    </div>
  `;
...
async function refreshBurnoutMain(){
  const teamEl = document.getElementById('burnoutTeamSection');
  const weekEl = document.getElementById('burnoutWeekSection');
  const scoreEl = document.getElementById('burnoutScoreSection');
  if (!teamEl || !weekEl || !scoreEl) return;
  ...
  if (empErr) {
    teamEl.innerHTML = `<div class="panel pad"><div style="font-weight:900">Your Team</div><div class="small" style="margin-top:6px;color:var(--muted)">Failed to load employees.</div></div>`;
    weekEl.innerHTML = '';
    scoreEl.innerHTML = '';
    return;
  }
  ...
  const emps = Array.isArray(employees) ? employees : [];
  renderBurnoutTeamSection(teamEl, emps);
  await renderBurnoutWeekSection(weekEl, emps);
  await renderBurnoutScoreSection(scoreEl, emps);
}

function renderBurnoutTeamSection(root, employees){
  const emps = Array.isArray(employees) ? employees : [];
  ...
  const card = (e) => {
    ...
    const line = (label, value) => `
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px">
        <div style="font-size:11px;font-weight:900;color:#94a3b8;letter-spacing:0.08em">${escapeHtml(label)}</div>
        <div style="font-size:12px;font-weight:800;color:#0f172a">${escapeHtml(value || '—')}</div>
      </div>
    `;

    const details = collapsed ? '' : `
      <div style="margin-top:12px;display:grid;gap:8px">
        ${line('START DATE', isoDate(e.start_date))}
        ${line('BIRTH DATE', isoDate(e.birth_date))}
        ${line('LAST VACATION', isoDate(e.last_vacation))}
      </div>
      <div style="margin-top:12px;display:flex;gap:10px;flex-wrap:wrap">
        <button type="button" onclick="window.editBurnoutEmployee('${id}')" style="background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.12);border-radius:12px;padding:10px 14px;font-size:12px;font-weight:900;color:#0f172a;cursor:pointer">Edit</button>
        <button type="button" onclick="window.deleteBurnoutEmployee('${id}')" style="background:rgba(255,107,107,0.08);border:1px solid rgba(255,107,107,0.22);border-radius:12px;padding:10px 14px;font-size:12px;font-weight:900;color:#FF6B6B;cursor:pointer">Delete</button>
      </div>
    `;

    return `
      <div style="background:rgba(255,255,255,0.75);border:1px solid rgba(0,0,0,0.08);border-radius:16px;padding:14px 16px">
        <div style="display:flex;align-items:center;justify-content:space-between;gap:12px">
          <button type="button" onclick="window.toggleBurnoutEmployeeCollapse('${id}')" style="background:none;border:none;padding:0;text-align:left;cursor:pointer;min-width:0;flex:1">
            <div style="font-family:Syne;font-weight:900;color:#0f172a;font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHtml(e.full_name || 'Employee')}</div>
          </button>
          <button type="button" onclick="window.toggleBurnoutEmployeeCollapse('${id}')" style="background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.12);border-radius:999px;padding:6px 10px;font-size:11px;font-weight:900;color:#64748b;cursor:pointer;flex-shrink:0">${collapsed ? 'Expand' : 'Collapse'}</button>
        </div>
        ${details}
      </div>
    `;
  ...
  root.innerHTML = `
    ...
    <div style="margin-top:12px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px">
      ${empty ? `
        <div style="grid-column:1/-1;background:rgba(0,0,0,0.02);border:1px dashed rgba(0,0,0,0.14);border-radius:16px;padding:18px">
          <div style="font-weight:900;color:#0f172a">Add your team members to get started</div>
          <div class="small" style="margin-top:6px;color:#64748b;font-weight:700;line-height:1.55">Employees are saved to your account and never need to be re-entered.</div>
        </div>
      ` : emps.map(card).join('')}
    </div>
  `;
...
async function renderBurnoutWeekSection(root, employees){
  ...
  if (emps.length) {
    ...
    for (const r of (Array.isArray(rows) ? rows : [])) {
      metricsByEmployeeId[String(r.employee_id)] = r;
    }
  }

  const hasAny = Object.keys(metricsByEmployeeId).length > 0;

  const rowHtml = (e) => {
    const m = metricsByEmployeeId[String(e.id)] || {};
    const v = {
      weekly_hours: Number.isFinite(Number(m.weekly_hours)) ? Number(m.weekly_hours) : '',
      weekend_hours: Number.isFinite(Number(m.weekend_hours)) ? Number(m.weekend_hours) : '',
      after_hours_messages: Number.isFinite(Number(m.after_hours_messages)) ? Number(m.after_hours_messages) : '',
      sick_days: Number.isFinite(Number(m.sick_days)) ? Number(m.sick_days) : ''
    };

    return `
      <div style="display:grid;grid-template-columns:minmax(140px,1.2fr) repeat(4, minmax(120px, 1fr));gap:10px;align-items:center;background:rgba(255,255,255,0.70);border:1px solid rgba(0,0,0,0.08);border-radius:14px;padding:12px">
        <div style="font-weight:900;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHtml(e.full_name || 'Employee')}</div>
        <input data-metric="weekly_hours" data-employee-id="${escapeHtml(String(e.id))}" type="number" min="0" step="1" value="${escapeHtml(String(v.weekly_hours))}" placeholder="Weekly hours" style="width:100%;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.10);border-radius:10px;padding:10px 12px;font-size:12px;color:#0f172a;font-weight:800;outline:none;box-sizing:border-box" />
        <input data-metric="weekend_hours" data-employee-id="${escapeHtml(String(e.id))}" type="number" min="0" step="1" value="${escapeHtml(String(v.weekend_hours))}" placeholder="Weekend hours" style="width:100%;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.10);border-radius:10px;padding:10px 12px;font-size:12px;color:#0f172a;font-weight:800;outline:none;box-sizing:border-box" />
        <input data-metric="after_hours_messages" data-employee-id="${escapeHtml(String(e.id))}" type="number" min="0" step="1" value="${escapeHtml(String(v.after_hours_messages))}" placeholder="After-hours msgs" style="width:100%;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.10);border-radius:10px;padding:10px 12px;font-size:12px;color:#0f172a;font-weight:800;outline:none;box-sizing:border-box" />
        <input data-metric="sick_days" data-employee-id="${escapeHtml(String(e.id))}" type="number" min="0" step="1" value="${escapeHtml(String(v.sick_days))}" placeholder="Sick days" style="width:100%;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.10);border-radius:10px;padding:10px 12px;font-size:12px;color:#0f172a;font-weight:800;outline:none;box-sizing:border-box" />
      </div>
    `;
  };
  ...
  root.innerHTML = `
    <div class="panel pad">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap">
        <div>
          <div style="font-family:Syne;font-weight:800;font-size:18px">This Week's Data</div>
          <div class="small" style="margin-top:6px">Week of <span style="font-weight:900;color:#FF6B6B">${escapeHtml(weekLabel)}</span></div>
        </div>
        <button type="button" onclick="window.saveBurnoutWeeklyData()" style="background:rgba(255,107,107,0.08);border:1px solid rgba(255,107,107,0.22);border-radius:12px;padding:12px 18px;font-size:13px;font-weight:900;color:#FF6B6B;cursor:pointer">${hasAny ? 'Update' : 'Save weekly data'}</button>
      </div>
    </div>
    ...
    <div style="margin-top:12px">
      ${emps.length ? `
        <div style="display:grid;gap:10px">
          <div style="display:grid;grid-template-columns:minmax(140px,1.2fr) repeat(4, minmax(120px, 1fr));gap:10px;padding:0 2px">
            <div style="font-size:10px;font-weight:900;color:#94a3b8;letter-spacing:0.10em">EMPLOYEE</div>
            <div style="font-size:10px;font-weight:900;color:#94a3b8;letter-spacing:0.10em">WEEKLY HOURS</div>
            <div style="font-size:10px;font-weight:900;color:#94a3b8;letter-spacing:0.10em">WEEKEND HOURS</div>
            <div style="font-size:10px;font-weight:900;color:#94a3b8;letter-spacing:0.10em">AFTER-HOURS MSGS</div>
            <div style="font-size:10px;font-weight:900;color:#94a3b8;letter-spacing:0.10em">SICK DAYS</div>
          </div>
          ${emps.map(rowHtml).join('')}
          <div id="burnoutWeeklyMsg" class="small" style="color:var(--muted)"></div>
        </div>
      ` : `
        <div style="background:rgba(0,0,0,0.02);border:1px dashed rgba(0,0,0,0.14);border-radius:16px;padding:18px">
          <div style="font-weight:900;color:#0f172a">No employees yet</div>
          <div class="small" style="margin-top:6px;color:#64748b;font-weight:700;line-height:1.55">Add employees in “Your Team” to unlock weekly updates.</div>
        </div>
      `}
    </div>
  `;

  window.__burnoutState.currentWeekStart = weekStart;
}

async function renderBurnoutScoreSection(root, employees){
  const emps = Array.isArray(employees) ? employees : [];
  if (!emps.length) {
    root.innerHTML = '';
    return;
  ...
  const last8 = weekStarts.slice(Math.max(0, weekStarts.length - 8));
  const series = last8.map(wk => {
    const list = byWeek[wk] || [];
    const scores = list.map(computeBurnoutScoreFromWeeklyMetrics);
    const avg = scores.length ? (scores.reduce((a,b)=>a+b,0) / scores.length) : 0;
    return Math.round(avg);
  });

  const latestWeekStart = last8[last8.length - 1];
  const latestMetrics = byWeek[latestWeekStart] || [];
  const latestEmployeeScores = latestMetrics.map(m => ({
    employee_id: m.employee_id,
    score: computeBurnoutScoreFromWeeklyMetrics(m)
  }));

  const companyScore = series[series.length - 1] ?? 0;
  const prev = series.length >= 2 ? series[series.length - 2] : null;
  const arrow = prev == null ? '—' : getTrendArrow(companyScore, prev);

  const atRiskCount = latestEmployeeScores.filter(e => ['medium','high','critical'].includes(computeRiskLevelFromScore(e.score))).length;
  const atRiskPct = latestEmployeeScores.length ? Math.round((atRiskCount / latestEmployeeScores.length) * 100) : 0;
  const topDriver = computeTopDriverFromMetrics(latestMetrics);

  root.innerHTML = `
    <div class="panel pad">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap">
        <div>
          <div style="font-family:Syne;font-weight:800;font-size:18px">Weekly Score & Trend</div>
          <div class="small" style="margin-top:6px">Shown after at least 1 weekly update exists.</div>
        </div>
      </div>
    </div>
    ...
    <div style="margin-top:12px;background:rgba(255,255,255,0.75);border:1px solid rgba(0,0,0,0.08);border-radius:16px;padding:16px">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-wrap:wrap">
        <div>
          <div style="font-size:11px;font-weight:900;color:#64748b;letter-spacing:0.08em">COMPANY BURNOUT SCORE</div>
          <div style="margin-top:8px;font-family:'Syne',system-ui;font-weight:900;font-size:52px;line-height:1;background:var(--home-grad2);-webkit-background-clip:text;background-clip:text;color:transparent">${escapeHtml(String(companyScore))}</div>
        </div>
    ...
    <div style="margin-top:12px;display:grid;grid-template-columns:repeat(4,1fr);gap:10px">
      <div style="background:rgba(255,107,107,0.06);border:1px solid rgba(255,107,107,0.18);border-radius:14px;padding:12px">
        ...
        <div style="display:flex;align-items:baseline;justify-content:space-between;margin-top:8px">
          <div style="font-family:'Syne',system-ui;font-weight:900;font-size:28px;color:#FF6B6B">${escapeHtml(String(companyScore))}</div>
          <div style="font-size:12px;font-weight:900;color:#0f172a">${escapeHtml(String(arrow))}</div>
        </div>
      </div>

      <div style="background:rgba(99,102,241,0.06);border:1px solid rgba(99,102,241,0.18);border-radius:14px;padding:12px">
        <div style="font-size:10px;font-weight:900;color:#6366f1;letter-spacing:0.08em">% AT RISK</div>
        <div style="font-family:'Syne',system-ui;font-weight:900;font-size:28px;color:#6366f1;margin-top:8px">${escapeHtml(String(atRiskPct))}%</div>
      ...
      <div style="background:rgba(0,184,148,0.06);border:1px solid rgba(0,184,148,0.18);border-radius:14px;padding:12px">
        ...
        <div style="font-size:13px;font-weight:900;color:#0f172a;margin-top:10px">${escapeHtml(String(topDriver))}</div>
      </div>
      ...
      <div style="background:rgba(0,0,0,0.03);border:1px solid rgba(0,0,0,0.08);border-radius:14px;padding:12px">
        ...
        <div style="font-size:22px;font-weight:900;color:#0f172a;margin-top:8px">${escapeHtml(String(arrow))}</div>
...
window.toggleBurnoutEmployeeCollapse = function(id){
  window.__burnoutState = window.__burnoutState || { collapsed: {}, weekInputs: {} };
  const key = String(id);
  window.__burnoutState.collapsed[key] = !window.__burnoutState.collapsed[key];
  refreshBurnoutMain();
};
...
window.editBurnoutEmployee = async function(id){
  try{
    const { data: { session: s } } = await supabase.auth.getSession();
    if (!s) return;
    ...
    const fullName = prompt('Employee full name', emp.full_name || '');
    if (!fullName || !String(fullName).trim()) return;
    const startDate = prompt('Start date (YYYY-MM-DD)', isoDate(emp.start_date));
    const birthDate = prompt('Birth date (YYYY-MM-DD)', isoDate(emp.birth_date));
    const lastVacation = prompt('Last vacation date (YYYY-MM-DD)', isoDate(emp.last_vacation));
    ...
    console.warn('Edit employee failed', e);
  }
};

window.deleteBurnoutEmployee = async function(id){
  try{
    const ok = confirm('Delete this employee? This will also delete their weekly metrics.');
    if (!ok) return;
    const { data: { session: s } } = await supabase.auth.getSession();
    if (!s) return;

    await supabase.from('weekly_metrics').delete().eq('employee_id', id);
    const { error } = await supabase.from('employees').delete().eq('id', id).eq('user_id', s.user.id);
    if (error) throw error;
    await refreshBurnoutMain();
  }catch(e){
    console.warn('Delete employee failed', e);
  }
};

window.saveBurnoutWeeklyData = async function(){
  const msgEl = document.getElementById('burnoutWeeklyMsg');
  ...
  try{
    ...
    const weekStart = String(window.__burnoutState?.currentWeekStart || isoDate(getMondayWeekStart(new Date())));

    const inputs = Array.from(document.querySelectorAll('#burnoutWeekSection input[data-employee-id][data-metric]'));
    const byEmp = {};
    for (const el of inputs) {
      const empId = String(el.getAttribute('data-employee-id') || '');
      const key = String(el.getAttribute('data-metric') || '');
      if (!empId || !key) continue;
      if (!byEmp[empId]) byEmp[empId] = {};
      const num = el.value === '' ? null : Number(el.value);
      byEmp[empId][key] = (num == null || !Number.isFinite(num)) ? null : num;
    }
    ...
    for (const r of (Array.isArray(existing) ? existing : [])) {
      existingByEmp[String(r.employee_id)] = r;
    }

    const inserts = [];
    const updates = [];
    for (const empId of empIds) {
      const values = byEmp[empId] || {};
      const weeklyHours = Number(values.weekly_hours ?? 0) || 0;
      const overtimeHours = Math.max(0, weeklyHours - 40);
      const payload = {
        employee_id: empId,
        week_start: weekStart,
        weekly_hours: values.weekly_hours ?? 0,
        weekend_hours: values.weekend_hours ?? 0,
        after_hours_messages: values.after_hours_messages ?? 0,
        sick_days: values.sick_days ?? 0,
        overtime_hours: overtimeHours
      };

      if (existingByEmp[empId]?.id) {
        updates.push({ id: existingByEmp[empId].id, ...payload });
      } else {
    ...
    if (inserts.length) {
      const r = await supabase.from('weekly_metrics').insert(inserts);
      if (r.error) throw r.error;
    }

    for (const u of updates) {
      const r = await supabase.from('weekly_metrics').update({
        week_start: u.week_start,
        weekly_hours: u.weekly_hours,
        weekend_hours: u.weekend_hours,
        after_hours_messages: u.after_hours_messages,
        sick_days: u.sick_days,
    ...
    console.warn('Save weekly metrics failed', e);
    if (msgEl) msgEl.textContent = 'Save failed.';
  }
};

async function refreshBurnoutAiFromWeek(weekStart){
  try{
    const { data: { session: s } } = await supabase.auth.getSession();
    if (!s) return;
    ...
    const metricsByEmp = {};
    for (const m of (Array.isArray(metrics) ? metrics : [])) metricsByEmp[String(m.employee_id)] = m;

    const employeesInput = emps.map(e => {
      const m = metricsByEmp[String(e.id)] || {};
      return {
        name: String(e.full_name || 'Employee'),
        ...
        lastVacation: isoDate(e.last_vacation) || 'unknown'
      };
    });
    ...
    const mergedEmployees = data.employees.map(e => {
      const k = String(e?.name || '').toLowerCase();
      const metricsRow = inputByName[k] || {};
      return { ...metricsRow, ...e };
    });

    pulseLast = { ...data, employees: mergedEmployees };
    window.__lastPulseEmployees = mergedEmployees;

    try{
      const atRisk = mergedEmployees.filter(e => ['high','critical'].includes(String(e.riskLevel||'').toLowerCase())).length;
      await insertAnalysisHistory('pulse', { employees: mergedEmployees, at_risk_count: atRisk });
    }catch(e){ /* noop */ }
    ...
    try{
      await supabase.from('pulse_results').insert({
        ...
        at_risk_count: mergedEmployees.filter(e => ['high','critical'].includes(String(e.riskLevel||'').toLowerCase())).length
      });
    }catch(e){ /* noop */ }

    window.__lastDecisionEngine = computeDecisionEngine(mergedEmployees, getCompanyTrendSeries(8));
    updateNavBurnoutBadge(window.__lastDecisionEngine);
  }catch(e){
    console.warn('AI refresh from weekly metrics failed', e);
  }
}
